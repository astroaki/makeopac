      SUBROUTINE BHMIE_ANGGRID(X,REFREL,NANG,THETA,S1,S2,
     &                         QEXT,QSCA,QBACK,GSCA)
C CPD&AK: Added a parameter: the Theta grid. Changed the name to
C         BHMIE_ANGGRID
C         NOTE: THETA is ONLY the grid between 0 and 90 degrees,
C               with THETA(1) = 0. and THETA(NANG) = 90.
...
C Arguments:
      ...
      DOUBLE PRECISION THETA(MXNANG)
      COMPLEX REFREL
      COMPLEX S1(2*MXNANG-1),S2(2*MXNANG-1)
C Local variables:
      INTEGER J,JJ,N,NSTOP,NMX,NN
      DOUBLE PRECISION CHI,CHI0,CHI1,DX,EN,FN,P,PII,PSI,PSI0,PSI1,
     &                 XSTOP,YMOD
...
C Modified by C.P.Dullemond and A.Kataoka to allow flexible theta
C     grid from 0 to 90 degrees (mirroring to 180 to 90) 16/11/01
C     Marked always with CPD&AK
...
C 16/11/01 (CPD&AK): Now taking an scattering angular grid THETA(:)
C***********************************************************************
C*** Safety checks
...
C CPD&AK: Here we simply use the theta grid given. 16/11/01
      DO 1000 J=1,NANG
          AMU(J)=COS(THETA(J))
 1000 CONTINUE
      DO 1100 J=1,NANG
